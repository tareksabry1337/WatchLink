...
struct FrameTests {
    ...
    func messageFrame() throws {
        let frame = try Frame(wrapping: PingMessage(count: 1), encoder: encoder, confirmedAcks: [])
    ...
    func uniqueIDs() throws {
        let f1 = try Frame(wrapping: PingMessage(count: 1), encoder: encoder, confirmedAcks: [])
        let f2 = try Frame(wrapping: PingMessage(count: 2), encoder: encoder, confirmedAcks: [])
        #expect(f1.id != f2.id)
    }

    @Test("message payload decodes back to original")
    func payloadRoundTrip() throws {
        let frame = try Frame(wrapping: PingMessage(count: 42), encoder: encoder, confirmedAcks: [])
    ...
    func controlPayloadRoundTrip() throws {
        let frame = try Frame(control: .pong, encoder: encoder, confirmedAcks: [])
    ...
    func messageRoundTrip() throws {
        let original = try Frame(wrapping: PongMessage(count: 7), encoder: encoder, confirmedAcks: [])
    ...
    func controlRoundTrip() throws {
        let original = try Frame(control: .pong, encoder: encoder, confirmedAcks: [])
        let data = try encoder.encode(original)
        let decoded = try decoder.decode(Frame.self, from: data)
